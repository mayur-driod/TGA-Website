...
export default function SignUpPage() {
  ...
  const handleMagicLink = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!normalizedName || !normalizedEmail) {
      setLocalError("Please provide your name and email address.")
    ...
    if (!signUpResponse.ok) {
      ...
      setLocalError(data?.message ?? "Unable to create your account right now. Please try again.")
      setEmailSent(null)
      setIsEmailLoading(false)
      return
    }
  ...
  return (
    <section className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-sm md:p-8">
      ...
      <h1 className="text-center text-2xl font-semibold tracking-tight text-foreground">Create your TGA account</h1>
      <p className="mt-2 text-center text-sm text-muted-foreground">
        Sign up with Google or create your account with a secure magic link.
      </p>
      ...
      <p className="mt-6 text-center text-xs text-muted-foreground">
        Already have an account? Use the sign-in page to request a magic link.
      </p>
